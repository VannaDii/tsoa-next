...
describe('CompilerOptions', () => {
  ...
  it('throws for an explicitly configured missing tsconfig', () => {
    ...
    ).to.throw(`Failed to read tsconfig at '${join(testDir, 'missing.tsconfig.json')}'`)
  })

  it('throws for invalid tsconfig json', async () => {
    await writeFile(join(testDir, 'tsconfig.json'), '{ "compilerOptions": ', 'utf8')

    expect(() => validateCompilerOptions(testConfig(), testDir)).to.throw(`Failed to read tsconfig at '${join(testDir, 'tsconfig.json')}'`)
  })
  ...
  it('throws for broken tsconfig extends', async () => {
    ...
    expect(() => validateCompilerOptions(testConfig(), testDir)).to.throw(`Failed to resolve tsconfig at '${join(testDir, 'tsconfig.json')}'`)
  })
